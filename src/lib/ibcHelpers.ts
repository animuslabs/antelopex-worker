...
// export function findActionType() {
...
//   const ibcToken = await getIBCToken(type.destinationChain, sym)
//   let native = ibcToken.native_chain.toString() == chain.config.chain
...
//   const ibcTkn = await getIBCToken(fromChain, Asset.Symbol.from(sym))
//   let nativeToken = ibcTkn.native_chain.toString() === fromChain.config.chain
//   if (!sym) throwErr("missing sym: " + tx_id)

//   // if (!receiver && !nativeToken) receiver = ibcTokens[sym].tokenContract[fromChain.config.chain]

//   if (!receiver && !nativeToken) receiver = ibcTkn.token_contract.toString()
//   console.log("sym", sym)
