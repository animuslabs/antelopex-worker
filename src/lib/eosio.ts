...
export async function getAllScopes(params:API.v1.GetTableByScopeParams) {
  ...
  if (!code) code = env.contracts.system
...
export async function doAction(name:NameType, data:{ [key:string]:any } = {}, contract:NameType = env.contracts.system, authorization?:PermissionLevel[], keys?:PrivateKey[], retry?:number):Promise<DoActionResponse | null> {
  if (!data) data = {}
